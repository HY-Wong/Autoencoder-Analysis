...
if __name__ == '__main__':
    ...
    # build the model
    config = load_config('imagenet_480M/stage1/config.yaml')
    config.arch = augment_arch_defaults(config.arch)
    
    vae, _ = create_model(config.arch)
    vae = vae.to(args.device)
    ckpt = torch.load('imagenet_480M/stage1/model.pt', map_location='cpu')['state_dict']
    ...
    if is_main_process():
        ...
        visualize_imgs.save(f'{save_dir}/recon_rq-vae-f32c256v16384.png')
