...
if __name__ == '__main__':
    ...
    parser.add_argument('--quantize_mode', default='kl', type=str)
    parser.add_argument('--latent', default='f4c3', type=str)
    ...
    # build the model
    config = OmegaConf.load(f'models/first_stage_models/{args.quantize_mode}-{args.latent}/config.yaml')
    vae = instantiate_from_config(config.model)
    ckpt = torch.load(f'checkpoints/{args.quantize_mode}-{args.latent}.ckpt', map_location='cpu', weights_only=False)['state_dict']
    ...
    if is_main_process():
        ...
        visualize_imgs.save(f'{save_dir}/recon_{args.quantize_mode}-vae-{args.latent}-ldm.png')
        ...
        if args.latent in ['f16c256v1024', 'f16c256v16384']:
            dataset = 'ImageNet'
        else:
            dataset = 'OpenImages'

        new_row = {
            'Model': f'{args.quantize_mode.upper()}-VAE-{args.latent}-LDM',
            'Dataset': dataset,
