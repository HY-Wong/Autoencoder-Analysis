...
if __name__ == '__main__':
    ...
    parser.add_argument('--latent_and_quantize', default='ll32_vae_c16', type=str)
    ...
    # build the model
    if is_main_process():
        ae = TiTok.from_pretrained(
            f'yucornetto/tokenizer_titok_{args.latent_and_quantize}_imagenet',
    ...
    match = re.search(r'\d+', args.latent_and_quantize)
    if match:
        compression_factor = 256 // int(match.group())
    if ae.quantize_mode == 'vq':
        latent = f'f256x{compression_factor}c16'
    elif ae.quantize_mode == 'vae':
        latent = f'f256x{compression_factor}c64v8192'
    else:
        latent = None
    ...
    with torch.no_grad():
        for imgs, labels in tqdm(image_val_loader, disable=not is_main_process(), desc='Processing images', leave=True):
            ...
            imgs_ = (imgs + 1) / 2 # work on range [0.0, 1.0]
            if ae.quantize_mode == 'vq':
                z, _ = ae.encode(imgs_)
            elif ae.quantize_mode == 'vae':
                _, posteriors = ae.encode(imgs_)
                z = posteriors.sample()
            rec_imgs_ = ae.decode(z)
            rec_imgs = rec_imgs_ * 2 - 1
   ...
   # aggregate losses across all distributed processes
    ...
    if is_main_process():
        ...
        visualize_imgs.save(f'{save_dir}/recon_titok-{ae.quantize_mode}-{latent}.png')
        ...
        new_row = {
            'Model': f'TiTok-{ae.quantize_mode.upper()}-{latent}',
            'Dataset': 'ImageNet',
