...
if __name__ == '__main__':
    ...
    parser.add_argument('--ft', default='ft-mse', type=str)
    ...
    if is_main_process():
        vae = AutoencoderKL.from_pretrained(
            f'stabilityai/sd-vae-{args.ft}', cache_dir='./checkpoints',
        ).to(args.device)
        torch.distributed.barrier()
    else:
        torch.distributed.barrier()
        vae = AutoencoderKL.from_pretrained(
            f'stabilityai/sd-vae-{args.ft}', cache_dir='./checkpoints', local_files_only=True,
        ).to(args.device)
    vae.eval()
    ...
    with torch.no_grad():
        for imgs, labels in tqdm(image_val_loader, disable=not is_main_process(), desc='Processing images', leave=True):
            ...
            posterior = vae.encode(imgs, return_dict=False)[0]
            z = posterior.sample()
            rec_imgs = vae.decode(z, return_dict=False)[0]
    ...
    if is_main_process():
        ...
        visualize_imgs.save(f'{save_dir}/recon_kl-vae-f8c4-{args.ft}.png')
        ...
        new_row = {
            'Model': f'KL-VAE-f8c4-{args.ft}',
            'Dataset': 'OpenImages, LAION-Aesthetics, LAION-Humans',
